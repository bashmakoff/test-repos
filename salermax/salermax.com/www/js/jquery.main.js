...
function initPlugin(){
	$('.counter').counter();
	$('.rules-section').mCustomScrollbar({
		scrollButtons:{
			enable:true
		}
	});
...
function changeView(){
	var main = $('#main'),
		...
		switchBtns = nav.find('.change-view'),
		activeClass = 'active',
		alternativeClass = 'alt',
		normalViewClass = 'normal-view',
		alternativeViewClass = 'alternative-view',
		navItemActive = navItems.filter('.' + activeClass);
	if(navItemActive.length){
		var currentBtn = navItemActive.find('a');
		if(currentBtn.attr('href') === alternativeViewClass){
			main.addClass(alternativeClass);
		}
	}
	switchBtns.click(function(){
		var currentBtn = $(this);
		navItems.removeClass(activeClass);
		currentBtn.parent('li').addClass(activeClass);
		if(currentBtn.attr('href') === normalViewClass){
			main.removeClass(alternativeClass);
		}
		else if(currentBtn.attr('href') === alternativeViewClass){
			main.addClass(alternativeClass);
		}
		return false;
	})
...
function showMessage(id){
	...
	if($('.popup').filter(':visible').length){
		$('.popup').filter(':visible').hide(animSpeed);
	}
	popup.show(animSpeed);
}

function hideMessage(id){
	var popup = $(id),
		animSpeed = 500;
	popup.hide(animSpeed);
}
...
function pressedState(){
	var el = $('.counter a, .btn-confirmation, #nav ul li a, .footer-nav ul li a'),
